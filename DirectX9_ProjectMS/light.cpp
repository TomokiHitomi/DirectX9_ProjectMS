...
void InitLight(void)
{
	...
	//// D3DLIGHT9構造体を0でクリアする
	//ZeroMemory(&g_aLight[6], sizeof(D3DLIGHT9));

	//// ライト6のタイプの設定
	//g_aLight[6].Type = D3DLIGHT_DIRECTIONAL;

	//// ライト6の拡散光の設定
	//g_aLight[6].Diffuse = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);

	//// ライト6の鏡面反射光の設定
	////	g_aLight[6].Specular = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);

	//// ライト6の環境光の設定
	////	g_aLight[6].Ambient = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);

	//// ライト6の方向の設定
	//vecDir = D3DXVECTOR3(0.0f, -100.0f, 0.0f);
	//D3DXVec3Normalize((D3DXVECTOR3*)&g_aLight[6].Direction, &vecDir);

	//// ライト6をレンダリングパイプラインに設定
	//pDevice->SetLight(6, &g_aLight[6]);
