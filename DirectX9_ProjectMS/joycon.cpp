...
void pollLoop()
{
	...
	for (int i = 0; i < size; ++i) {
		...
		if (g_pRumble[i].bUse)
		{
			jc->rumble(g_pRumble[i].nFrequency, g_pRumble[i].nIntensity);
			g_pRumble[i].bUse = false;
			jc->rumble(10, 3);
		}
...
init_start:
	...
	// 振動用構造体をJoyconの数インスタンス化
	g_pRumble = new Rumble[joycons.size()];
...
D3DXVECTOR3 GetJoyconAccel(int jcNo)
{
	...
	D3DXVECTOR3 vecTemp = D3DXVECTOR3(jc->accel.x / 10, jc->accel.y / 10, jc->accel.z / 10);
	return vecTemp;
}
...
void JoyconUpdate(void)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (jc->left_right == 1)
		{
			...
			if (bGui)
			{
				ImGui::Text(u8"【Joy-Con Left】No[%d]\n",i);
		...
		if (jc->left_right == 2)
		{
			...
			if (bGui)
			{
				ImGui::Text(u8"【Joy-Con Right】No[%d]\n", i);
...
// 振動設定：frequency 0-255  intensity 0-8
