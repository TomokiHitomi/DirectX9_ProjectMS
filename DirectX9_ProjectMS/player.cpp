...
void Player::Float(void)
{
	...
	D3DXVECTOR3 jcL, jcR;
	jcL = GetJoyconAccel(0);
	jcR = GetJoyconAccel(1);

	//if (jcL.x < 0.95f && jcR.x < 0.95f)
	//{
		if (jcL.y > MOVE_MARGIN && jcR.y > MOVE_MARGIN)
		{
			m_vPos -= m_vZ * (m_fMoveSpeed * CompHigh(jcL.y, jcR.y));
		}
		else if (jcL.y < -MOVE_MARGIN && jcR.y < -MOVE_MARGIN)
		{
			m_vPos -= m_vZ * (m_fMoveSpeed * CompHigh(jcL.y, jcR.y));
		}
		if (jcL.z > MOVE_MARGIN && jcR.z > MOVE_MARGIN)
		{
			m_vPos += m_vX * (m_fMoveSpeed * CompHigh(jcL.z, jcR.z));
		}
		else if (jcL.z < -MOVE_MARGIN && jcR.z < -MOVE_MARGIN)
		{
			m_vPos += m_vX * (m_fMoveSpeed * CompHigh(jcL.z, jcR.z));
		}
